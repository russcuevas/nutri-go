...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10"
    ...
    <div class="mb-8">
        ...
        <p class="text-xs text-gray-500 mt-1">Pinpoint your exact delivery location on the map. Distance, live fare, and ETA calculate dynamically.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-card space-y-4">
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <span class="text-[10px] font-extrabold uppercase tracking-wider text-nutri-600">Step 1</span>
                            <h3 class="text-lg font-black text-gray-900 font-heading">Pinpoint Your Exact Delivery Location</h3>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="hidden sm:inline-flex px-2.5 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-bold items-center gap-1">
                                <i class="fa-solid fa-hand-pointer text-emerald-600"></i> Drag Pin or Tap Map
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="relative w-full h-80 rounded-2xl overflow-hidden border border-gray-200 shadow-inner">
                        ...
                        <button type="button" onclick="window.useCurrentGpsLocation()" title="Detect My Current GPS Location"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                        <div>
                            ...
                            <input type="text" name="recipient_name" required value="{{ auth()->user()->name ?? '' }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-nutri-500" placeholder="e.g. Maria Clara">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Recipient Mobile Phone *</label>
                            <input type="tel" name="recipient_phone" required value="{{ auth()->user()->phone ?? '' }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-nutri-500" placeholder="09171234567">
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1 flex items-center justify-between">
                            <span>Address</span>
                            <span class="text-[10px] text-gray-400 font-medium normal-case flex items-center gap-1"><i class="fa-solid fa-lock text-[9px]"></i> Auto-detected from map pin</span>
                        </label>
                        <input type="text" name="delivery_address" id="delivery_address" x-model="address" readonly required class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-700 text-xs outline-none font-semibold cursor-default" placeholder="Pin location on map to detect address">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Delivery Landmark *</label>
                            <input type="text" name="delivery_landmark" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-nutri-500" placeholder="e.g Such as your House no. etc ..">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Rider Notes / Instructions (Optional)</label>
                            <input type="text" name="delivery_notes" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-nutri-500" placeholder="e.g. Leave at guard house / Ring doorbell 2x">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-card space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <label class="relative flex items-center p-4 rounded-2xl border-2 cursor-pointer transition"
                               :class="deliveryType === 'immediate' ? 'border-nutri-600 bg-nutri-50/50 ring-1 ring-nutri-600' : 'border-gray-200 hover:border-gray-300'">
                            <input type="radio" name="delivery_type" value="immediate" x-model="deliveryType" class="sr-only">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-limey-400 text-nutri-950 flex items-center justify-center font-bold">
                        ...
                        <label class="relative flex items-center p-4 rounded-2xl border-2 cursor-pointer transition"
                               :class="deliveryType === 'scheduled' ? 'border-nutri-600 bg-nutri-50/50 ring-1 ring-nutri-600' : 'border-gray-200 hover:border-gray-300'">
                            <input type="radio" name="delivery_type" value="scheduled" x-model="deliveryType" class="sr-only">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-nutri-900 text-white flex items-center justify-center font-bold">
                    ...
                    <div x-show="deliveryType === 'scheduled'" x-cloak class="pt-2">
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Select Scheduled Delivery Date & Time</label>
                        <input type="datetime-local" name="scheduled_for" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-nutri-500">
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-card space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <label class="relative flex items-center p-4 rounded-2xl border-2 cursor-pointer transition"
                               :class="paymentMethod === 'gcash' ? 'border-blue-600 bg-blue-50/40 ring-1 ring-blue-600' : 'border-gray-200 hover:border-gray-300'">
                        ...
                        <label class="relative flex items-center p-4 rounded-2xl border-2 cursor-pointer transition"
                               :class="paymentMethod === 'cod' ? 'border-nutri-600 bg-nutri-50/40 ring-1 ring-nutri-600' : 'border-gray-200 hover:border-gray-300'">
                            <input type="radio" name="payment_method" value="cod" x-model="paymentMethod" class="sr-only">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-bold text-sm">
                                    ₱
                                </div>
                                <div>
                    ...
                    <!-- GCash Upload Box -->
                    <div x-show="paymentMethod === 'gcash'" class="p-5 rounded-2xl bg-blue-50/60 border border-blue-200 space-y-4 text-xs">
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-white rounded-xl border border-blue-100 shadow-sm">
                            <div>
                                <span class="text-[10px] font-extrabold uppercase text-blue-600 tracking-wider">Store GCash Account</span>
                            ...
                            @if($store->gcash_qr_code)
                                <div class="w-16 h-16 rounded-lg overflow-hidden border border-gray-200 shrink-0">
                                    <img src="{{ $store->gcash_qr_code }}" alt="QR Code" class="w-full h-full object-cover">
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[11px] font-bold text-gray-700 uppercase mb-1">13-Digit GCash Reference No. *</label>
                                <input type="text" name="payment_reference_no" placeholder="e.g. 1002938475819" class="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-xs bg-white outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-gray-700 uppercase mb-1">Upload Receipt Screenshot *</label>
                                <input type="file" name="payment_proof_image" accept="image/*" class="w-full text-xs text-gray-500 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="bg-white p-6 rounded-3xl border border-nutri-200 shadow-card space-y-4 sticky top-24">
                    <h3 class="text-xs font-extrabold uppercase tracking-widest text-nutri-900 flex items-center justify-between">
                        <span class="flex items-center gap-2">
                            <i class="fa-solid fa-receipt text-nutri-600"></i> Live Fare Summary
                        </span>
                        <span class="text-[10px] text-emerald-700 bg-emerald-100 font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Realtime GPS
                        </span>
                    </h3>

                    <!-- Realtime Distance Calculation Box -->
                    <div class="p-4 rounded-2xl bg-nutri-50 border border-nutri-200 space-y-2.5 text-xs transition-all duration-300">
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Point-to-Point Distance:</span>
                            <span class="font-extrabold text-sm text-gray-900 bg-white px-2.5 py-0.5 rounded-lg border border-nutri-200 shadow-sm" x-text="distance.toFixed(2) + ' km'"></span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Estimated Delivery ETA:</span>
                            <span class="font-bold text-xs text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded border border-emerald-200" x-text="'~' + etaMins + ' mins'"></span>
                        </div>
                        ...
                        <div class="flex justify-between pt-2 border-t border-nutri-200 font-black text-sm text-nutri-800">
                            <span>Delivery Total:</span>
                            <span class="text-emerald-700 font-extrabold text-base" x-text="'₱' + deliveryFee.toFixed(2)"></span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 pt-2 border-t border-gray-100 text-xs">
                        ...
                        <div class="flex justify-between pt-3 border-t border-gray-200 text-base font-black text-gray-900 font-heading">
                            <span>Grand Total:</span>
                            <span class="text-nutri-700 text-xl font-black" x-text="'₱' + (subtotal + deliveryFee + platformFee - discount).toFixed(2)"></span>
                        </div>
                    </div>
                    ...
                    <button type="submit" 
                        ...
                        <span class="flex items-center gap-2"><i class="fa-solid fa-lock"></i> Place Order & Dispatch</span>
                    </button>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.useCurrentGpsLocation = function() {
            ...
            if (btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin text-limey-400"></i> Locating...';
            ...
            navigator.geolocation.getCurrentPosition(
                function(pos) {
                    ...
                    if (btn) {
                        btn.innerHTML = '<i class="fa-solid fa-circle-check text-emerald-400"></i> Located!';
                        setTimeout(() => {
                ...
                function(err) {
                    ...
                    alert("Unable to detect GPS position. Please allow Location permissions in your browser.");
                    if (btn) btn.innerHTML = originalHtml;
                },
