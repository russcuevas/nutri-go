            ...
            <div class="lg:col-span-8 space-y-4">
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-card">
                    <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                        <div>
                            <span class="text-[10px] font-extrabold uppercase tracking-wider text-nutri-600">Ordering From</span>
                            <h3 class="text-lg font-black text-gray-900 font-heading">{{ $store->store_name ?? 'Healthy Partner' }}</h3>
                        </div>
                        ...
                        @foreach($cart as $item)
                            <div class="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-center gap-4">
                                    <img src="{{ $item['image'] }}" alt="{{ $item['name'] }}" class="w-16 h-16 rounded-2xl object-cover bg-gray-50 border border-gray-100 shrink-0">
                                    <div>
                                ...
                                <div class="flex items-center justify-between sm:justify-end gap-6">
                                    <form action="{{ route('users.cart.update') }}" method="POST" class="flex items-center border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
                                        @csrf
                                        <input type="hidden" name="product_id" value="{{ $item['id'] }}">
                                        <button type="submit" name="quantity" value="{{ $item['quantity'] - 1 }}" class="px-3 py-1 text-xs text-gray-600 hover:bg-gray-200 transition font-bold">-</button>
                                        <span class="px-3 py-1 text-xs font-bold text-gray-900 bg-white">{{ $item['quantity'] }}</span>
                                        <button type="submit" name="quantity" value="{{ $item['quantity'] + 1 }}" class="px-3 py-1 text-xs text-gray-600 hover:bg-gray-200 transition font-bold">+</button>
                                    </form>
                ...
                <div class="bg-white p-6 rounded-3xl border border-nutri-200 shadow-card space-y-4">
                    ...
                    <div class="p-4 rounded-2xl bg-nutri-50 border border-nutri-200 grid grid-cols-2 gap-4 text-center">
                        <div>
                            ...
                            <div class="text-2xl font-black text-nutri-900 font-heading">{{ $totalCalories }} <span class="text-xs text-nutri-600 font-bold">kcal</span></div>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 font-semibold">Total Protein</div>
                            <div class="text-2xl font-black text-rose-600 font-heading">{{ $totalProtein }} <span class="text-xs text-rose-500 font-bold">g</span></div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 pt-2 border-t border-gray-100 text-xs">
                        ...
                        <div class="flex justify-between text-gray-600">
                            ...
                            <span class="text-nutri-600 font-semibold">Computed at Lipa pin</span>
                        </div>
                    </div>

                    <a href="{{ route('users.checkout.index') }}" class="w-full py-4 rounded-2xl bg-limey-400 hover:bg-limey-500 text-nutri-950 font-black text-sm shadow-md transition font-heading flex items-center justify-center gap-2">
                        Proceed to Map Checkout <i class="fa-solid fa-arrow-right"></i>
                    </a>
                </div>
            </div>
        </div>
