...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <a href="{{ route('store.orders.index') }}" class="text-xs font-bold text-nutri-600 hover:text-nutri-700 flex items-center gap-1.5">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-card space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-gray-100 gap-4">
            ...
            <div class="text-right">
                ...
                <div class="text-2xl font-black text-nutri-900 font-heading">₱{{ number_format($order->total_amount, 2) }}</div>
            </div>
        </div>

        <!-- Payment Verification Details & Receipt Inspection -->
        <div class="p-5 rounded-2xl {{ $order->payment_method === 'gcash' ? 'bg-blue-50/70 border-blue-200' : 'bg-emerald-50/70 border-emerald-200' }} border space-y-4">
            <div class="flex items-center justify-between">
                <h4 class="text-xs font-black uppercase tracking-wider text-gray-900 flex items-center gap-1.5">
                    <i class="fa-solid fa-receipt text-blue-600"></i> Payment Details ({{ strtoupper($order->payment_method) }})
                </h4>
                <span class="text-xs font-bold uppercase {{ $order->payment_status === 'verified' ? 'text-emerald-700' : 'text-amber-700' }}">
            ...
            @if($order->payment_method === 'gcash')
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div>
                        ...
                        <div class="text-sm font-mono font-black text-blue-900 mt-0.5">{{ $order->payment_reference_no ?? 'No reference entered' }}</div>
                    </div>
                    <div>
                        <span class="text-gray-500 font-semibold">Receipt Screenshot:</span>
                        @if($order->payment_proof_url)
                            <div class="mt-1">
                                <a href="{{ $order->payment_proof_url }}" target="_blank" class="inline-flex items-center gap-1 text-xs font-bold text-blue-700 hover:underline">
                                ...
                                <div class="w-32 h-32 rounded-xl overflow-hidden border mt-1 bg-gray-900">
                                    <img src="{{ $order->payment_proof_url }}" alt="Proof" class="w-full h-full object-cover">
                                </div>
                            </div>
                        @else
            ...
            @else
                <p class="text-xs text-emerald-800">Cash on Delivery: Delivery rider will collect ₱{{ number_format($order->total_amount, 2) }} from customer upon arrival.</p>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs text-gray-700">
            ...
            <div class="space-y-2">
                ...
                @if($order->delivery_landmark)
                    <div>Landmark / House No.: <span class="font-semibold text-gray-900">{{ $order->delivery_landmark }}</span></div>
                @endif
        ...
        @if($order->status === 'pending_store')
            <div class="pt-4 border-t flex gap-3">
                <form action="{{ route('store.orders.accept', $order->id) }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md transition">
                        Accept Order & Start Preparation
                    </button>
                </form>
            </div>
        @elseif($order->status === 'store_accepted_preparing')
            <div class="pt-4 border-t">
                <form action="{{ route('store.orders.ready', $order->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-2">
