    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                @if ($recipe->is_premium_only)
                    <span
                        class="px-3 py-1 rounded-full bg-amber-400 text-nutri-950 text-xs font-black uppercase tracking-wider">
                        👑 VIP Exclusive Masterclass
                    </span>
                @else
                    <span
                        class="px-3 py-1 rounded-full bg-rose-100 text-rose-800 text-xs font-extrabold uppercase tracking-wider">
                        🎬 Free Cooking Guide
                    </span>
                @endif
            ...
            <div class="flex items-center gap-3 pt-2">
                <img src="{{ $recipe->creator->avatar_url }}" alt="{{ $recipe->creator->name }}"
        ...
        <div class="rounded-3xl overflow-hidden shadow-2xl bg-black border border-gray-800 relative">
            @if ($recipe->is_premium_only && !$isVip)
                <!-- VIP Paywall Overlay -->
                <div
                    class="relative h-96 flex flex-col items-center justify-center p-8 text-center text-white bg-gradient-to-b from-nutri-950/90 to-black">
                    <img src="{{ $recipe->thumbnail_url }}" alt="Locked"
                        class="absolute inset-0 w-full h-full object-cover opacity-20 blur-sm">
                    <div class="relative z-10 max-w-md space-y-4">
                        <div
                            class="w-16 h-16 rounded-3xl bg-amber-400 text-nutri-950 text-2xl flex items-center justify-center mx-auto shadow-xl">
                            <i class="fa-solid fa-lock"></i>
                        </div>
                        <h3 class="text-2xl font-black font-heading">NutriGo VIP Exclusive Video</h3>
                        <p class="text-xs text-gray-300 leading-relaxed">
                            This full masterclass and customized macro guide is exclusive to NutriGo VIP members. Upgrade
                            today to unlock all video recipes!
                        </p>
                        <a href="{{ route('users.subscriptions.index') }}"
                            class="inline-flex px-6 py-3.5 rounded-2xl bg-amber-400 hover:bg-amber-300 text-nutri-950 font-black text-xs shadow-xl transition font-heading transform hover:scale-105">
        ...
        <div class="p-6 rounded-3xl bg-nutri-50 border border-nutri-200 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
            <div>
                ...
                <div class="text-2xl font-black text-nutri-900 font-heading">🔥 {{ $recipe->calories }} <span
                        class="text-xs font-semibold text-gray-500">kcal</span></div>
            </div>
            <div>
                <span class="text-[10px] font-extrabold uppercase text-gray-500">Protein</span>
                <div class="text-2xl font-black text-rose-600 font-heading">💪 {{ $recipe->protein_g }} <span
                        class="text-xs font-semibold text-gray-500">g</span></div>
            </div>
            <div>
                <span class="text-[10px] font-extrabold uppercase text-gray-500">Carbs</span>
                <div class="text-2xl font-black text-amber-600 font-heading">🍞 {{ $recipe->carbs_g }} <span
                        class="text-xs font-semibold text-gray-500">g</span></div>
            </div>
            <div>
                <span class="text-[10px] font-extrabold uppercase text-gray-500">Healthy Fats</span>
                <div class="text-2xl font-black text-emerald-600 font-heading">🥑 {{ $recipe->fat_g }} <span
                        class="text-xs font-semibold text-gray-500">g</span></div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-5 space-y-6">
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-card space-y-4">
                    <div class="flex items-center justify-between pb-3 border-b border-gray-100">
                        <h3 class="text-sm font-black font-heading text-gray-900 uppercase tracking-wider">Required
                            Ingredients</h3>
            ...
            <div class="lg:col-span-7 space-y-6">
                <div class="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200/80 shadow-card space-y-4">
                    <h3
                        class="text-sm font-black font-heading text-gray-900 uppercase tracking-wider pb-3 border-b border-gray-100">
