...
<div x-data="{
...
}" @open-system-tutorial.window="isOpen = true; currentStep = 1" x-show="isOpen" x-cloak
    ...
    <div @click.away="close()" x-transition:enter="transition ease-out duration-300"
        ...
        <!-- Top Header -->
        <div
            class="px-6 py-5 bg-gradient-to-r from-nutri-950 via-nutri-900 to-emerald-950 text-white flex items-center justify-between relative overflow-hidden">
            <div class="absolute -right-10 -top-10 w-36 h-36 bg-limey-400/20 rounded-full blur-2xl pointer-events-none">
            </div>

            <div class="flex items-center gap-3 relative z-10">
                <div
                    class="w-10 h-10 rounded-xl bg-gradient-to-br from-limey-400 to-emerald-500 text-nutri-950 flex items-center justify-center font-black text-lg shadow">
                    🌱
                </div>
                <div>
                    <div class="flex items-center gap-2">
                        <span
                            class="text-[10px] font-black uppercase tracking-wider text-limey-300 bg-white/10 px-2 py-0.5 rounded-full">
                            System Guide
                        </span>
                        <span class="text-xs text-nutri-200 font-semibold"
                            x-text="'Hakbang ' + currentStep + ' ng ' + totalSteps"></span>
                    </div>
            ...
            <button @click="close()"
                class="relative z-10 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition hover:scale-105 active:scale-95">
        ...
        <div class="p-6 sm:p-8 min-h-[340px] flex flex-col justify-between">
            ...
            <div x-show="currentStep === 1" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center gap-3">
                    <span
                        class="w-12 h-12 rounded-2xl bg-nutri-50 text-nutri-700 text-2xl flex items-center justify-center shrink-0">
                        👤
                    </span>
                    <div>
                        <span class="text-[11px] font-black text-nutri-600 uppercase tracking-wider">Hakbang 1: Paggawa
                            ng Account</span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
                    <div class="p-3.5 rounded-xl bg-nutri-50 border border-nutri-100">
                        ...
                        <p class="text-[11px] text-gray-500 mt-1">Umorder ng calorie-counted food at sumali sa VIP prep.
                        </p>
                    </div>
                    <div class="p-3.5 rounded-xl bg-emerald-50 border border-emerald-100">
                        ...
                        <p class="text-[11px] text-gray-500 mt-1">Magbenta ng healthy dishes at health supplements sa
                            Lipa.</p>
                    </div>
                    <div class="p-3.5 rounded-xl bg-limey-50 border border-limey-200">
                        <p class="text-xs font-bold text-nutri-900">🛵 Rider</p>
                        <p class="text-[11px] text-gray-500 mt-1">Mag-deliver gamit ang smart GPS trip routing at PIN
                            security.</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="currentStep === 2" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center gap-3">
                    <span
                        class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 text-2xl flex items-center justify-center shrink-0">
                        🥗
                    </span>
                    <div>
                        <span class="text-[11px] font-black text-emerald-600 uppercase tracking-wider">Hakbang 2:
                            Pagpili ng Pagkain</span>
            ...
            <div x-show="currentStep === 3" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center gap-3">
                    <span
                        class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-700 text-2xl flex items-center justify-center shrink-0">
                        🛒
                    </span>
                    <div>
                        <span class="text-[11px] font-black text-amber-600 uppercase tracking-wider">Hakbang 3:
                            Pag-order</span>
                        <h4 class="text-xl font-black text-gray-900 font-heading">Cart at Special Notes</h4>
                    </div>
                </div>
                <p class="text-xs sm:text-sm text-gray-600 leading-relaxed">
                    I-click ang <strong>Add to Cart</strong>. Bawat order ay nakapaloob sa isang tindahan para
                    manatiling mainit at sariwa ang pagkain pagdating sa iyo.
                </p>
            ...
            <div x-show="currentStep === 4" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center gap-3">
                    <span
                        class="w-12 h-12 rounded-2xl bg-blue-50 text-blue-700 text-2xl flex items-center justify-center shrink-0">
                        💳
                    </span>
                    <div>
                        <span class="text-[11px] font-black text-blue-600 uppercase tracking-wider">Hakbang 4:
                            Pagbabayad</span>
                        <h4 class="text-xl font-black text-gray-900 font-heading">COD Checkout</h4>
                    </div>
                </div>
                <p class="text-xs sm:text-sm text-gray-600 leading-relaxed">
                ...
                <p class="text-[11px] text-gray-500">Ang mga VIP Club members ay awtomatikong may libreng delivery fee
                    sa Lipa City.</p>
            </div>
            ...
            <div x-show="currentStep === 5" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <span class="text-[11px] font-black text-blue-600 uppercase tracking-wider">Hakbang 5:
                            Pagdating ng Pagkain</span>
                        <h4 class="text-xl font-black text-gray-900 font-heading">Live Map Tracking ng Assigned Rider
                        </h4>
                    </div>
                </div>
                <p class="text-xs sm:text-sm text-gray-600 leading-relaxed">
                    Aabangan mo lang ang iyong order sa mapa! Makikita mo ang real-time na biyahe ng iyong assigned
                    rider patungo sa iyong lokasyon sa Lipa.
                </p>
                <div class="p-4 rounded-2xl bg-gray-900 text-white flex items-center justify-between">
                    ...
                    <div
                        class="px-3 py-1.5 rounded-xl bg-limey-400/20 border border-limey-400/30 text-limey-300 font-bold text-xs flex items-center gap-1.5">
                        <i class="fa-solid fa-motorcycle text-limey-400"></i> No PIN Needed
                    </div>
                </div>
                <p class="text-[11px] text-gray-500 leading-relaxed">
                    Walang PIN na kailangan—diretso nang iaabot ng rider ang sariwang pagkain pagdating sa iyong
                    address.
                </p>
            </div>
            ...
            <div class="pt-6 mt-6 border-t border-gray-100 flex items-center justify-between">
                ...
                <!-- Buttons -->
                <div class="flex items-center gap-2">
                    <button type="button" x-show="currentStep > 1" @click="prev()"
                        class="px-4 py-2 rounded-xl border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-50 transition">
                        Bumalik
                    </button>

                    <button type="button" x-show="currentStep < totalSteps" @click="next()"
                        class="px-5 py-2.5 rounded-xl bg-nutri-900 hover:bg-nutri-800 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm">
                    ...
                    <div x-show="currentStep === totalSteps" class="flex items-center gap-2">
                        <a href="{{ route('docs.guide') }}"
                            class="px-3.5 py-2 rounded-xl border border-nutri-300 text-nutri-800 text-xs font-bold hover:bg-nutri-50 transition">
                            Full Docs
                        </a>
                        <button type="button" @click="close()"
                            class="px-5 py-2.5 rounded-xl bg-limey-400 hover:bg-limey-500 text-nutri-950 text-xs font-black transition shadow-sm font-heading">
