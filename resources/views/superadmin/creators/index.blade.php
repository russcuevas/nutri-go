        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($creators as $c)
                    <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-between gap-3 group hover:border-gray-200 transition">
                        ...
                        <form action="{{ route('superadmin.creators.destroy', $c->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete creator {{ addslashes($c->name) }} and all their recipes?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-2 rounded-xl text-gray-400 hover:text-rose-600 hover:bg-rose-50 transition shrink-0 cursor-pointer" title="Delete Creator">
        ...
        @else
            <div class="divide-y divide-gray-100">
                @foreach($recipes as $v)
                    <div class="py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:bg-gray-50/50 px-2 rounded-2xl transition">
                        <!-- Thumbnail & Info -->
                        ...
                        <div class="flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-gray-100 text-gray-600 font-bold text-xs">
                                <i class="fa-regular fa-eye text-gray-400"></i>
                                {{ number_format($v->views_count) }} Views
                            </span>
