            ...
            <div class="py-5 flex flex-col md:flex-row md:items-center justify-between gap-4" x-data="{ openRejectModal: false }">
                <div class="space-y-1 text-xs">
                    ...
                    <p class="text-gray-700">
                        Vehicle: <span class="font-semibold text-gray-900">{{ $r->vehicle_type }}</span> • Plate: <span class="font-mono font-bold">{{ $r->plate_number }}</span>
                    </p>
                    <p class="text-gray-500">
                        License: <span class="font-mono font-bold text-gray-800">{{ $r->license_number }}</span> • Phone: {{ $r->phone }} • Base Brgy: <span class="font-bold text-nutri-800">{{ $r->barangay }}</span>
                    </p>
                    @if($r->rejection_reason)
                        <p class="text-rose-600 font-semibold italic">Rejection Reason: "{{ $r->rejection_reason }}"</p>
                    @endif
                </div>
                ...
                <div x-show="openRejectModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
                    <div @click.outside="openRejectModal = false" class="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
                        <h4 class="font-black font-heading text-base text-gray-900">Decline Rider Application: {{ $r->user->name }}</h4>
                        <form action="{{ route('superadmin.riders.reject', $r->id) }}" method="POST" class="space-y-4 text-xs">
                            @csrf
                            <div>
                                <label class="block font-bold text-gray-700 mb-1">Reason for Rejection *</label>
                                <textarea name="rejection_reason" required rows="3" class="w-full p-3 rounded-xl border border-gray-200 text-xs" placeholder="e.g. Invalid driver's license number..."></textarea>
                            </div>
